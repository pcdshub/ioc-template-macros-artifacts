...
epicsEnvSet("IOC", "ioc-xpp-pico-las01")
epicsEnvSet("IOCSH_PS1", "$(IOC)> ")
epicsEnvSet("EPICS_NAME", "IOC:XPP:LAS:PIC")
epicsEnvSet("IOCTOP","/cds/group/pcds/epics/ioc/common/picoMotion/R4.0.1")
epicsEnvSet("TOP","/reg/g/pcds/epics/ioc/xpp/picoMotion/R1.0.0/build")
epicsEnvSet("STREAM_PROTOCOL_PATH", "$(IOCTOP)/protocol" )
epicsEnvSet("IP", "mcf-xpp-las01")
epicsEnvSet("Port", "P0")
epicsEnvSet("CtrlBase", "M")
epicsEnvSet("PREFIX", "XPP:LAS:PIC")
...
dbLoadRecords("db/basic_asyn_motor.db","P=$(PREFIX):,M=01,DTYP='asynMotor',DIR=Pos,INIT='',DESC=tt input H,PORT=$(CtrlBase)0,ADDR=0,$(OPEN_MM)")
dbLoadRecords("db/basic_asyn_motor.db","P=$(PREFIX):,M=02,DTYP='asynMotor',DIR=Pos,INIT='',DESC=tt input V,PORT=$(CtrlBase)0,ADDR=1,$(OPEN_MM)")
dbLoadRecords("db/basic_asyn_motor.db","P=$(PREFIX):,M=03,DTYP='asynMotor',DIR=Pos,INIT='',DESC=tt output H,PORT=$(CtrlBase)0,ADDR=2,$(OPEN_MM)")
dbLoadRecords("db/basic_asyn_motor.db","P=$(PREFIX):,M=04,DTYP='asynMotor',DIR=Pos,INIT='',DESC=tt output V,PORT=$(CtrlBase)0,ADDR=3,$(OPEN_MM)")
