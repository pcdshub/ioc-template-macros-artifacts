...
epicsEnvSet( "TPR_PV",       "LAS:LHN:BAY1:CAM:01:NoTpr" )
epicsEnvSet( "EVR_PV",       "EVR:LAS:LHN:01:NF" )

# Setup EVR env vars
epicsEnvSet( "TRIG_PV",      "$(EVR_PV):TRIG9" )
...
epicsEnvSet( "CAM_IP",       "172.21.160.90" )
epicsEnvSet( "CAM_PV",       "LAS:LHN:BAY1:CAM:01" )
...
dbLoadRecords( "$(EVRDB)", "IOC=$(IOC_PV),EVR=$(EVR_PV),CARD=$(EVR_CARD),IP9E=Enabled" )

# Load both devEvrInfo.db and devTprInfo.db so python programs can test for both sets of PVs
dbLoadRecords( "db/devEvrInfo.db",				"DEV=$(CAM_PV),EVR=$(EVR_PV),TRIG_CH=9,EVR_USED=1" )
