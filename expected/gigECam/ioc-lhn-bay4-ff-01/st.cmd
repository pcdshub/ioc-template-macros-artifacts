#!/reg/g/pcds/epics/ioc/common/gigECam/R5.0.3/bin/rhel7-x86_64/gige

# Run common startup commands for linux soft IOC's
< $(IOC_COMMON)/All/pre_linux.cmd

epicsEnvSet( "BUILD_TOP",    "/reg/g/pcds/epics/ioc/las/gigECam/R0.3.6/build" )
< envPaths
epicsEnvSet( "IOCNAME",      "ioc-lhn-bay4-ff-01" )
epicsEnvSet( "ENGINEER",     "Tyler Johnson (tjohnson)" )
epicsEnvSet( "LOCATION",     "Laser Hall Alignment FF camera" )
epicsEnvSet( "IOCSH_PS1",    "$(IOCNAME)> " )
epicsEnvSet( "IOC_PV",       "IOC:LAS:LHN:BAY4:CAM:02" )
...
epicsEnvSet( "TPR_PV",       "LAS:LHN:BAY4:CAM:02:NoTpr" )
epicsEnvSet( "EVR_PV",       "EVR:LAS:LHN:04:FF" )

# Setup EVR env vars
epicsEnvSet( "TRIG_PV",      "$(EVR_PV):TRIG7" )
...
epicsEnvSet( "CAM_IP",       "172.21.160.174" )
epicsEnvSet( "CAM_PV",       "LAS:LHN:BAY4:CAM:02" )
epicsEnvSet( "CAM_PORT",     "CAM" )
epicsEnvSet( "MODEL",        "Basler_acA1440-73gm" )
...
dbLoadRecords( "$(EVRDB)", "IOC=$(IOC_PV),EVR=$(EVR_PV),CARD=$(EVR_CARD),IP7E=Enabled" )

# Load both devEvrInfo.db and devTprInfo.db so python programs can test for both sets of PVs
dbLoadRecords( "db/devEvrInfo.db",				"DEV=$(CAM_PV),EVR=$(EVR_PV),TRIG_CH=7,EVR_USED=1" )
...
dbpf $(CAM_PV):LAUNCH_EDM "/reg/g/pcds/epics/ioc/las/gigECam/R0.3.6/build/iocBoot/$(IOCNAME)/edm-$(IOCNAME).cmd"
