...
$$LOOP(IPIMB)
dbLoadRecords( "db/ipimb.db", "RECNAME=$$NAME,BOX=$$TRANSLATE(NAME,":","-"),EVR=$$EVRNAME,TRIG=$$TRIG,FLNK=$$NAME:BSAFANOUT")
dbLoadRecords("db/evrDevInfo.db", "BASE=$$NAME,EVR=$$EVRNAME,TRIG=$$TRIG,NAME=$$NAME")
dbLoadRecords("db/BSAfanout.db", "RECNAME=$$NAME$$IF(BSA),L1=$$NAME:EFSUM$$ENDIF(BSA)$$IF(BSA1),L2=$$NAME:EF$$BSA1$$ENDIF(BSA1)$$IF(BSA2),L3=$$NAME:EF$$BSA2$$ENDIF(BSA2)$$IF(BSA3),L4=$$NAME:EF$$BSA3$$ENDIF(BSA3)$$IF(BSA4),L5=$$NAME:EF$$BSA4$$ENDIF(BSA4)$$IF(BSA5),L6=$$NAME:EF$$BSA5$$ENDIF(BSA5)")

$$IF(BSA)
dbLoadRecords( "db/ipimbBSA.db", "RECNAME=$$NAME,ATTR=SUM")
$$ENDIF(BSA)
$$IF(BSA1)
dbLoadRecords( "db/ipimbBSA.db", "RECNAME=$$NAME,ATTR=$$BSA1")
$$ENDIF(BSA1)
$$IF(BSA2)
dbLoadRecords( "db/ipimbBSA.db", "RECNAME=$$NAME,ATTR=$$BSA2")
$$ENDIF(BSA2)
$$IF(BSA3)
dbLoadRecords( "db/ipimbBSA.db", "RECNAME=$$NAME,ATTR=$$BSA3")
$$ENDIF(BSA3)
$$IF(BSA4)
dbLoadRecords( "db/ipimbBSA.db", "RECNAME=$$NAME,ATTR=$$BSA4")
$$ENDIF(BSA4)
$$IF(BSA5)
dbLoadRecords( "db/ipimbBSA.db", "RECNAME=$$NAME,ATTR=$$BSA5")
$$ENDIF(BSA5)
$$ENDLOOP(IPIMB)
