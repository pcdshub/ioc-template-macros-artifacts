#!$$IOCTOP/bin/$$IF(ARCH,$$ARCH,linux-86_64)/rssmx100x
...
$$LOOP(SMB)
$$IF(HOST)
drvAsynIPPortConfigure( "bus$$INDEX", "$$HOST:$$PORT TCP", 0, 0, 0 )
$$ELSE(HOST)
drvAsynSerialPortConfigure( "bus$$INDEX", "/dev/$$PORT", 0, 0, 0 )
$$ENDIF(HOST)
$$IF(ASYNTRACE)
...
$$LOOP(SMA)
dbLoadRecords( "db/SMA.db", "BASE=$$BASE, PORT=bus$$INDEX" )
#dbLoadRecords( "db/SMA.db", "BASE=$$BASE, DEV=bus$$INDEX" )
$$ENDLOOP(SMA)
$$LOOP(SMB)
dbLoadRecords( "db/SMB.db", "BASE=$$BASE, DEV=bus$$INDEX" )
$$ENDLOOP(SMB)
