#!$$IOCTOP/bin/$$IF(TARGET_ARCH,$$TARGET_ARCH,rhel7-x86_64)/axiRogue
...
# Specify AXI_PV
$$IF(AXI_PV)
epicsEnvSet( "AXI_PV",       "$$AXI_PV" )
$$ELSE(AXI_PV)
errlog( "AXI_PV not defined" )
$$ENDIF(AXI_PV)

# Specify AXI_PORT
epicsEnvSet( "AXI_PORT",     "$$IF(AXI_PORT,$$AXI_PORT,AXI)" )

# Default pgpCamlinkDriver settings
epicsEnvSet( "PGP_PV",       "$$IF(PGP_PV,$$PGP_PV,$(AXI_PV))" )
...
var DEBUG_AXI_ROGUE $$IF(DEBUG_AXI_ROGUE,$$DEBUG_AXI_ROGUE,1)
...
dbpf $(AXI_PV):LAUNCH_EDM "$$TOP/iocBoot/$(IOCNAME)/edm-$(IOCNAME).cmd"
