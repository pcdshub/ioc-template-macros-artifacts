#!$$IOCTOP/bin/$$IF(ARCH,$$ARCH,linux-x86_64)/evr
...
dbLoadRecords( "$(EVRDB)", "TYPE=$$DELAY_TYPE,IOC=$(IOC_PV),EVR=$(EVR_PV),CARD=$(EVR_CARD)$$LOOP(EXTRA_TRIG),IP$$(TRIG)E=Enabled$$ENDLOOP(EXTRA_TRIG)$$IF(TRIG),IP$$(TRIG)E=Enabled,$$ENDIF(TRIG)$$IF(ENABLE_ALL),IP0E=Enabled,IP1E=Enabled,IP2E=Enabled,IP3E=Enabled$$IF(EVR_TYPE,SLAC),IP4E=Enabled,IP5E=Enabled,IP6E=Enabled,IP7E=Enabled,IP8E=Enabled,IP9E=Enabled,IPAE=Enabled,IPBE=Enabled$$ENDIF(EVR_TYPE)$$ENDIF(ENABLE_ALL)")
