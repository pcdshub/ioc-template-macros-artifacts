...
$$LOOP(MOTOR)
dbLoadRecords( "$(TOP)/db/ims_long.db", "IOC=$(EPICS_NAME),MOTOR=$$NAME,PORT=$$PORT,ASYN=$$IF(ASYN,$$ASYN,$$NAME),DVER=$(DVER),ERBL=$$ERBL,DESC=$$IF(DESC,$$DESC,),TYPE=$$IF(TYPE,$$TYPE,),EE=$$IF(EE,$$EE,Disable),EL=$$IF(EL,$$EL,512),MT=$$IF(MT,$$MT,0),HT=$$IF(HT,$$HT,500),RCMX=$$IF(RCMX,$$RCMX,5),RC=$$IF(RC,$$RC,5),HCMX=$$IF(HCMX,$$HCMX,0),HC=$$IF(HC,$$HC,0),EGU=$$IF(EGU,$$EGU,),DIR=$$IF(DIR,$$DIR,Pos),SREV=$$IF(SREV,$$SREV,51200),UREV=$$IF(UREV,$$UREV,1),BDST=$$IF(BDST,$$BDST,0),HDST=$$IF(HDST,$$HDST,5),RDBD=$$IF(RDBD,$$RDBD,3),PDBD=$$IF(PDBD,$$PDBD,5),SBAS=$$IF(SBAS,$$SBAS,0.5),SMAX=$$IF(SMAX,$$SMAX,2.0),S=$$IF(S,$$S,1.0),BS=$$IF(BS,$$BS,1.0),HS=$$IF(HS,$$HS,1.0),ACCL=$$IF(ACCL,$$ACCL,1),BACC=$$IF(BACC,$$BACC,1),HACC=$$IF(HACC,$$HACC,1),DLLM=$$IF(DLLM,$$DLLM,0),DHLM=$$IF(DHLM,$$DHLM,0),FW_MEANS=$$IF(FW_MEANS,$$FW_MEANS,+),REV_MEANS=$$IF(REV_MEANS,$$REV_MEANS,-)" )
$$ENDLOOP(MOTOR)
